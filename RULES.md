...
Every release must look polished, aesthetic, and informative:
- **Release Title**: Always descriptive with themed emojis (e.g. `v0.1.0 - 📺 Autonomous Android TV Accessibility Keeper 🚀`). Never just a bare version tag.
- **Summary**: A concise paragraph explaining what the release accomplishes.
- **Key Highlights**: Bullet points with emojis categorizing features, improvements, and bug fixes.
- **Binary Assets**: Clear overview of architecture tarballs/binaries available for download.
- **1-Click Quick Install**: Copy-pasteable command snippet for immediate installation.
- **`CHANGELOG.md` Maintenance**: Keep a structured `CHANGELOG.md` so GitHub Actions release workflows automatically parse aesthetic release notes for each tag.
